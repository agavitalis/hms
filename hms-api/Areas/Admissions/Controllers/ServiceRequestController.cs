...
namespace HMS.Areas.Admissions.Controllers
{
    ...
    public class ServiceRequestController : Controller
    {
        ...
        [Route("PayForAdmissionService")]
        [HttpPost]
        public async Task<IActionResult> PayForAdmissionServices(AdmissionServiceRequestPaymentDto serviceRequest)
        {
            ...
            var admission = await _admission.GetAdmission(serviceRequest.AdmissionId);
            ...
            var servicesRequestCheck = await _admissionServiceRequest.CheckIfServiceRequestIdExist(serviceRequest.ServiceRequestId);
            ...
            var correctAmount = await _admissionServiceRequest.CheckIfAmountPaidIsCorrect(serviceRequest);
            ...
            try
            {
                var result = await _admissionServiceRequest.PayForServices(serviceRequest);
                if (!result)
                    return BadRequest(new
                    {
                        response = "301",
        ...
        [Route("PayForAdmissionServicesWithAccount")]
        [HttpPost]
        public async Task<IActionResult> PayForAdmissionServicesWithAccount(AdmissionServiceRequestPaymentDto serviceRequest)
        {
            if (serviceRequest == null)
            {
                return BadRequest(new { message = "Invalid Post attempt made" });
            ...
            //pay for services
            var result = await _admissionServiceRequest.PayForServicesWithAccount(serviceRequest);
            if (!result)
                return BadRequest(new
                {
                    response = "301",
