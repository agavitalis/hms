...
namespace HMS.Areas.NHIS.Repositories
{
    public class HMOHealthPlanServicePriceRepository : IHMOHealthPlanServicePrice
    {
        ...
        {
            var HMOHealthPlanServicePrices = _applicationDbContext.HMOHealthPlanServicePrices.Include(dp => dp.Service).ThenInclude(dp => dp.ServiceCategory).Include(dp => dp.HMOHealthPlan).Where(dp => dp.HMOHealthPlanId == HealthPlanId).OrderBy(dp => dp.Service.Name).ToList();
