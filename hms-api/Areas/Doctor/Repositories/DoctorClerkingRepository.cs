...
namespace HMS.Areas.Doctor.Repositories
{
    public class DoctorClerkingRepository : IDoctorClerking
    {
        ...
        public async Task<IEnumerable<DoctorClerking>> GetDoctorClerkingByPatient(string PatientId) => await _applicationDbContext.DoctorClerkings.Where(c => c.Appointment.PatientId == PatientId || c.Consultation.PatientId == PatientId).Include(c => c.Appointment.Doctor).ToListAsync();
        ...
        public async Task<DoctorClerking> GetDoctorClerkingByConsultation(string ConsultationId) => await _applicationDbContext.DoctorClerkings.Where(c => c.ConsultationId == ConsultationId).Include(c => c.Appointment.Doctor).FirstOrDefaultAsync();
